...
%When iterating through these followees, there was a chance that we would be unable to retrieve the necessary history in the considered time window we desired due to multiple reasons, such as the 3200 messages limit imposed by Twitter or errors in the communication. To account for that, the ego networks were also filtered in the following fashion: considering a 28 days window since the last post we crawled from the ego-user, if any of the users followees' history was partially or totally unavailable (the oldest message would be inside or after the beginning of the window), we would estimate the number of lost messages based on the period unavailable and the average posting rate for the particular followee. If the total estimation of lost messages was more than 20\% of the total number of messages that we would expect in the window\footnote{Other loss tolerance (5\%, 10\%, 50\%, 80\%, 100\%) were tested. Lower values lead to similar conclusions to the ones presented in this work, while higher values would increase the number of users that lacked data for the analysis. The 20\% loss tolerance was presented a reasonable trade-off in sample size and results consistency.} (the ones crawled and estimated), we discarded the ego network due to lack of data. This procedure guarantees that we have substantial history for most ego networks.

 %DC 16: For me, this figure doesn't add value for the space, so, cutting for length.
%\begin{figure}[!htbp]
%\centering
%\fontsize{6pt}{7pt}\selectfont
%\input{./figures/fig_twitter_dataset}
%\caption{The dashed line indicates the ego network for the \textit{follower} $flr_n$. The %%tweets for all encompassed nodes in each ego network were collected.} \label{fig:fig_twitter_dataset}
%\end{figure}
...
%\includegraphics{./figures/all_tfidf_histogram_cutoff_y.eps}
...
%It is possible to see that some of these users present a high activity on Twitter, posting hundreds of messages in the 14-days window considered in this work. The behavior of high activity users such as these is lost when considering only explicit system-tagged reactions. Also, it is unlikely that such users are unaware that the Reply and Retweet mechanisms exist on Twitter, suggesting that this behavior is characteristic of these users, and not only the naive usage of the system.
...
\end{figure}
%% DC 24: Thanks for making the x axes comparable in each row (which I realize looks a little weird for the 6-tweet user).  It's interesting just how much the upper right one looks like the total distribution of scores...
